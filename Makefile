...
test_vga_background:
	rm -rf sim_build/
	mkdir sim_build/
	iverilog -o sim_build/sim.vvp -s vga_background -s dump -g2012 src/vga_background.v test/dump_vga_background.v src/
...
test_vga_core:
	rm -rf sim_build/
	mkdir sim_build/
	iverilog -o sim_build/sim.vvp -s vga_core -s dump -g2012 src/vga_core.v src/vga_timing.v src/vga_background.v test/dump_vga_core.v src/
